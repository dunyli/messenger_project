...
// Тип текущего клиента: 0 = TCP, 1 = WebSocket
static int client_is_websocket = 0;

/*
 * send_to_client — отправляет ответ клиенту (TCP через send, WebSocket через ws_send)
 */
static void send_to_client(int client_fd, const char *status, const char *body) {
    char response[BUFFER_SIZE];
    protocol_build_response(status, body, response, sizeof(response));

    if (client_is_websocket) {
        ws_send(client_fd, response, strlen(response));
    } else {
...
static void handle_register(int client_fd, parsed_message_t *msg) {
    if (msg->args_count < 2) {
        send_to_client(client_fd, "ERROR", "Usage: REGISTER|login|password");
        return;
    }
    ...
    if (result == 1) {
        ...
        send_to_client(client_fd, "OK", "Регистрация успешна");
    } else if (result == 0) {
        send_to_client(client_fd, "ERROR", "Данный логин уже используется");
    } else {
        send_to_client(client_fd, "ERROR", "Database error");
    }
}
...
static void handle_login(int client_fd, parsed_message_t *msg) {
    if (msg->args_count < 2) {
        send_to_client(client_fd, "ERROR", "Usage: LOGIN|login|password");
        return;
    }
    ...
    if (user_id >= 0) {
        ...
        send_to_client(client_fd, "OK", response_body);
    } else {
        send_to_client(client_fd, "ERROR", "Неверный логин или пароль.");
    }
}
...
static void handle_msg(int client_fd, parsed_message_t *msg) {
    if (current_user_id < 0) {
        send_to_client(client_fd, "ERROR", "Not logged in");
        return;
    }

    if (msg->args_count < 2) {
        send_to_client(client_fd, "ERROR", "Usage: MSG|recipient|text");
        return;
    }
    ...
    if (PQresultStatus(res) != PGRES_TUPLES_OK || PQntuples(res) == 0) {
        ...
        send_to_client(client_fd, "ERROR", "Recipient not found");
        return;
    }
    ...
    if (chat_id < 0) {
        ...
        if (PQresultStatus(res) != PGRES_TUPLES_OK) {
            PQclear(res);
            send_to_client(client_fd, "ERROR", "Failed to create chat");
            return;
        }
        chat_id = atoi(PQgetvalue(res, 0, 0));
        PQclear(res);
    ...
    if (message_id < 0) {
        send_to_client(client_fd, "ERROR", "Failed to save message");
        return;
    }
...
static void handle_history(int client_fd, parsed_message_t *msg) {
    ...
    send_to_client(client_fd, "OK", "History printed on server console");
}
...
static void handle_logout(int client_fd) {
    ...
    send_to_client(client_fd, "OK", "Goodbye!");
}
...
void handle_client(int client_fd) {
    ...
    if (strncmp(buffer, "GET /", 5) == 0) {
        ...
        client_is_websocket = 0;
        send_to_client(client_fd, "OK", "Welcome to Messenger v1.0");
    }
    ...
    while (1) {
        ...
        if (!protocol_parse(buffer, &parsed)) {
            send_to_client(client_fd, "ERROR", "Unknown command or bad format");
            continue;
        }
